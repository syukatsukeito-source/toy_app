...
class PasswordResetForm < PasswordResets
  ...
  def setup
    super
    @user = users(:michael)
    @original_reset_digest = @user.reset_digest
    post password_resets_path,
         params: { password_reset: { email: @user.email } }
    @user.reload
    @mail = ActionMailer::Base.deliveries.last
    @reset_token = CGI.unescape(@mail.body.encoded.match(/password_resets\/([^\/]+)\/edit/)[1])
  end
end
...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with valid email" do
    assert_not_equal @original_reset_digest, @user.reset_digest
  ...
  test "reset with inactive user" do
    ...
    get edit_password_reset_path(@reset_token,
                                 email: @user.email)
    assert_redirected_to root_url
  end
